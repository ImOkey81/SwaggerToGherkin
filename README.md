...
это означает, что бинарник `swagger-codegen` не установлен в системе/контейнере.

В Docker-образе этого репозитория `swagger-codegen` теперь устанавливается автоматически.
После пересборки контейнера генерация тестов будет работать.
