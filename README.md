...
## Если при старте всё ещё видно `swagger-codegen-cli-3.0.46.jar!/`
Это означает, что запускается образ/код из старой ревизии, где `swagger-codegen-cli` был добавлен в зависимости приложения.

Проверьте:
1. Вы на актуальном коммите:
```
git pull
git log -1 --oneline
```
...
docker build --no-cache -t swagger_to_gherkin .
```
3. Убедитесь, что в финальном `app.jar` нет `BOOT-INF/lib/swagger-codegen-cli-...jar` (в Dockerfile добавлена автоматическая проверка; сборка упадёт, если jar попал внутрь).

4. Если в логе `docker build` у вас финальный stage показывает только `5/5` шагов и **нет шага проверки `jar tf app.jar ...`**, значит используется старый `Dockerfile` (проверьте ветку/коммит).
